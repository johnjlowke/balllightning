c trich14a has also rnp = 5.0e10, rni = 2.5e10, rne = 2.5e10
c trich14 inserts ball of metastables nm = 5.0e17
...
c boundary voltage on top plate beyong j = 20 to scale linearly to zero         
          voltr = volts
c          if (j.gt.20) voltr = volts*(r(nr)-r(j))/(r(nr)-r(20)) 
           if (j.le.20) voltr = volts*(r(20)-r(j))/(r(20)-r(1)) 
c          if (j.gt.80) voltr = 0.0 
          if (j.gt.20) voltr = 0.0 
          call zerocoef(dc,df,de,dw,dn,ds,voltr,j,1)
          call zerocoef(dc,df,de,dw,dn,ds,1.0d-7,j,nz)
 30   continue
       do 21 i=1,nz
         call zerocoef(dc,df,de,dw,dn,ds,1.0d-7,nr,i)
 21   continue
c       do 21 i=1,nz
