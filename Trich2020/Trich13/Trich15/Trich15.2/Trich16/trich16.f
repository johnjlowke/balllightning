c trich16 is for thundercloud radius 20m = 2000 cm
c trich15 is for sheath analysis for linearly changing ambient field 
c   Inserted ball of metastables on bckground positive and negative ions
c trich14 inserts ball of metastables
c trich13 has increments in denssities after particle rates
c trich13 has rnm(0,i) = rnm(1,i)
c trich 13 has radial and z axial velocities zero at radial and axial boundaries
c trich 13 has electron diffusion, trial for long time runs
c trich12a is developed copy of trich12 from venice to apple; trich 12 is for discharge from frame.
c trich7 has multiplication factor for metastable densities for i
c  greater than 33 or 93 cm; fact in arcin
c trich6b has correction to backward difference treatment of fluxes to
c  correct for cells over which there is a change in direction of field.
c trich6 is trich5c
...
c trichel3 has glass nonabsorbing or emitting of positive and negative particles through field at glass
...
c     fact is the factor metstable densities are increased
...
c initial metastable values changed by factor fact from arcin
      do 205 i = 1,nz
      do 206 j = 1,nr
        rnm(j,i)= rnm(j,i)*fact
 206  continue
 205  continue
c initial values in ball of positive and negative ions electrons and metastables, Fig7 J Phys D 1992
      do 207 i = 1,nz
      do 208 j = 1,nr
c       Radd = 10.0 value for Prague
c        Radd = 0.3
c        Radd = 2.0 
        Radd = 2000.0 
	xx=r(j)
	xx2 = xx**2
c	zz = z(i)-310.0   Prague value
c	zz = z(i)-0.45   
c	zz = z(i)-5.00   
	zz = z(i)-5000.00   
	zz2 = zz**2
        dist=sqrt(xx2+zz2)
	if(dist.le.Radd) then
           rnm(j,i) = 5.0e17
        else
           rnm(j,i) = 5.0e10
        endif
...
c      time = 0.0
 ...
 187  continue
      do 188 j=1,nr
          phi(j,0) = phi(j,1)
          wiz = -rmob*ez(j,1)
c          if(j.gt.20) wiz = 0.0
          wize = 100*wiz
c          if(ez(j,i).le.0.0) then
c              rjniz(j,0) = 0.0  
c              rjnez(j,0) = 0.0  
              rjniz(j,0) = wiz*rni(j,1) 
              rjnez(j,0) = wize*rne(j,1) 
c important line to insure positive ions absorbed at metal surface
              rjnpz(j,0) = -wiz*rnp(j,1)  
c          else
c              rjniz(j,0) = wiz*rni(j,1) 
c              rjnez(j,0) = wize*rne(j,1) 
c              rjnpz(j,0) = 0.0 
c          endif
 188  continue
c      write(*,850) m,resphi,jphi,iphi,relphi,jjphi,iiphi
      ...
      do 194 j=1,nr
          ...
          rece = 0.0
          reci = 0.0
c calculation of particle flux densities
c Radial current densities
          wir = -rmob*er(j,i)
c Radial currents at radial boundaries zero. Charges arise at insulators.       
          if (j.eq.nr) wir = 0.0
          wire = 100*wir
...
c Axial current densities
          wiz = -rmob*ez(j,i)
c Axial currents at anode zero. Charges arise at boundary for insulators.
          if (i.eq.nz) wiz = 0.0
          wize = 100*wiz
...
c calculation of particle rates
...
c          if (i.eq.33) eabs(j,i) = sqrt(ez(j,i)**2)
          eav = eabs(j,i)
          eon = eav/rn
          weav = 100*rmob*eav
...
c Add Diffusion of electrons; diff coefficient = diffelect
          fdifer = (r(j)+r(j+1))*(rne(j+1,i)-rne(j,i))/(2*r(j)*dr(j))
          bdifer = (r(j)+r(j-1))*(rne(j,i)-rne(j-1,i))/(2*r(j)*dr(j-1))
          diffel = diffelect*(((rne(j,i+1)-rne(j,i))/(z(i+1)-z(i))
     r   -(rne(j,i)-rne(j,i-1))/(z(i)-z(i-1)))/dz(i)
     r    +(fdifer-bdifer)/dr(j))
          dne(j,i) =  dne(j,i) + diffel
c          if (j.eq.20.and.i.eq.1)
c     r         write(*,*) j,i,diffel,rion,att,rece,det,rne(j,i)
c          if (j.eq.20.and.i.eq.1)
c     r         write(*,*) detach,rnm(j,i),rni(j,i) 
          if (rne(j,i).le.1.0E-7) rne(j,i) =1.0E-7
...
c         de(nr,i) = 0.d0
 ...
 40   continue
      do 30 j=1,nr
c boundary voltage on top i=1 is input voltage,at i=nz zero volts
          voltr = volts
          call zerocoef(dc,df,de,dw,dn,ds,voltr,j,1)
          call zerocoef(dc,df,de,dw,dn,ds,1.0d-7,j,nz)
 30   continue
       do 21 i=1,nz
         voltz = volts*(z(nz)-z(i)+z(1))/z(nz)
         call zerocoef(dc,df,de,dw,dn,ds,voltz,nr,i)
 21   continue
 520     format (5e16.8)
 501  format(/)
c	 write(305,501)
