...
c          if (i.eq.32) wir = 0.0
          ...
          sigg = er(j,i)*er(j+1,i)
c correction to backward difference method if field changes sign
          if (sigg.lt.0.0) then   
           rjnir(j,i)=wir*(rni(j,i)+rni(j+1,i))*0.5*(1+(dr(j)/(2*r(j))))
          rjner(j,i)=wire*(rne(j,i)+rne(j+1,i))*0.5*(1+(dr(j)/(2*r(j))))
          rjnpr(j,i)=-wir*(rnp(j,i)+rnp(j+1,i))*0.5*(1+(dr(j)/(2*r(j))))
          endif
          wiz = -rmob*ez(j,i)
...
c correction to backward difference method if field changes sign
          sigg = ez(j,i)*ez(j,i+1)
          if (sigg.lt.0.0) then   
           rjniz(j,i)=wiz*(rni(j,i)+rni(j,i+1))*0.5*(1+(dr(j)/(2*r(j))))
          rjnez(j,i)=wize*(rne(j,i)+rne(j,i+1))*0.5*(1+(dr(j)/(2*r(j))))
          rjnpz(j,i)=-wiz*(rnp(j,i)+rnp(j,i+1))*0.5*(1+(dr(j)/(2*r(j))))
          endif
