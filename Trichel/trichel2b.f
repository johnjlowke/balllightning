...
c      if iion=1 has ionization, irep is iterations per print out
...
c Trichel modification - positive ions impinging up on glass window are absorbed
          if (i.eq.32) rjnpz(j,32) = 0.0
          if (i.eq.33.and.ez(j,i).le.0.0) rjnpz(j,32) =
     r         +rmob*ez(j,32)*rnp(j,33)
c          if (i.eq.33.and.rjniz(j,i).le.0.0) rjniz(j,32) = rjniz(j,33)
c          if (i.eq.33.and.rjnez(j,i).le.0.0) rjnez(j,32) = rjnez(j,33)
c          if (i.eq.33.and.rjnpz(j,i).le.0.0) rjnpz(j,32) = rjnpz(j,33)
...
c make under surface at i=32 conducting
	  if (i.eq.33.and.rjniz(j,i).le.0.0) rjniz(j,i-1)=rjniz(j,i)
          dni(j,i) = -(rjnir(j,i)-rjnir(j-1,i))/dr(j) - 
     r        (rjniz(j,i)-rjniz(j,i-1))/dz(i)+att-reci - det
          if (i.ge.2) rni(j,i) = rni(j,i) + dni(j,i)*delt
c          if (i.eq.nz) rni(j,i) = rni(j,i-1)
c          if (j.eq.nr) rni(j,i) = rni(j-1,i)
c make under surface at i=32 conducting
	  if (i.eq.33.and.rjnez(j,i).le.0.0) rjnez(j,i-1)=rjnez(j,i)
          dne(j,i) = -(rjner(j,i)-rjner(j-1,i))/dr(j) - 
     r        (rjnez(j,i)-rjnez(j,i-1))/dz(i)+rion-att-rece + det
          if (i.ge.2) rne(j,i) = rne(j,i) + dne(j,i)*delt
c          if (i.eq.nz) rne(j,i) = rne(j,i-1)
c          if (j.eq.nr) rne(j,i) = rne(j-1,i)
...
c          if (i.eq.nz) rnp(i,j) = rnp(j,i-1) 
c          if (j.eq.nr) rnp(j,i) = rnp(j-1,i) 
...
c	  dnm(j,i) = rmet -det -qen
 ...
 197  continue
      ...
      if (icount.eq.irep) then
          write(8,852) time,rmaxne
          icount = 0
      endif
 200  continue
